package lista4;

import java.io.BufferedReader;
import java.io.InputStreamReader;
import java.io.OutputStream;
import java.net.HttpURLConnection;
import java.net.URL;
import java.nio.charset.StandardCharsets;
import javax.swing.JOptionPane;

public class Boleto{
    
    public static void boleto(){
        try{
            //Está pedindo a linha digitavel do boleto, que nesse caso é 23793381286008301352856000063307789840000150000.
            String linhaDigitavel = JOptionPane.showInputDialog(null, "Insira a linha digitavel do boleto");

            //URL da api mesmo
            URL url = new URL("https://sandbox.openfinance.celcoin.dev/v5/transactions/billpayments/authorize");
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();

            //JSON, pegar o JSON no insomnia e colar aqui. Caso dê algum problema, colar o JSON do insomnia no javaescaper e depois passar pra cá.
            String str = "{\r\n" +
                    "  \"barCode\": {\r\n" +
                    "    \"type\": 0,\r\n" +
                    "    \"digitable\": " +
                    "\"" + linhaDigitavel + "\"\r\n" +
                    "  }\r\n" +
                    "}";
        
            //Configurando a conexão, só colocar o que está no header do request em questão do insomnia.
            connection.setRequestMethod("POST");
            connection.setRequestProperty("Content-Type", "application/json");
            connection.setRequestProperty("Authorization", "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..FGjUl60wpfzppAXP2LuXFxx0DH0wjnjBiSiNJ9rCnU4");
            
            //Tem que fazer isso
            connection.setDoOutput(true);

            //Vai enviar a requisição.
            try (OutputStream os = connection.getOutputStream()){
                byte[] input = str.getBytes(StandardCharsets.UTF_8);
                os.write(input, 0, input.length);
            }
           
            //Lê a resposta do servidor.
            BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
            StringBuilder response = new StringBuilder();
            String line;

            while ((line = reader.readLine()) != null){
                response.append(line);
            }

            reader.close();
            connection.disconnect();

            //Replace my beloved. Adiciona quebras de linha em pontos específicos do JSON
            String respostaDiferente = response.toString().replace("\",\"", "\",\n\"");

            //Mostrando o boleto.
            JOptionPane.showMessageDialog(null, respostaDiferente);

        } 
        //Caso aconteça um erro, é uma mensagem de erro.
        catch (Exception e){
            JOptionPane.showMessageDialog(null, "ERRO");
        }
    }

}
